...
async function init() {
    ...
    if (!skyport) {
        ...
        if (!imageCheck) {
            log.error('before starting VOLQ Panel for the first time, you didn\'t run the seed command!');
        ...
        await db.set('volq_instance', info)
        log.info('initialized VOLQ Panel with id: ' + volqId)
    }        
